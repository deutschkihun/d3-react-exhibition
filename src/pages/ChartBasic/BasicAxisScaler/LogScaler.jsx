...
export function LogScaler({ width, height, margin, index }) {
  useEffect(() => {
    const xScaleLog = d3.scaleLog().domain([1, 1000]).range([0, width]).base(2)
    const yScaleLog = d3.scaleLog().domain([1, 1000]).range([height, 0]).base(2)
    const axisTemplate = d3
      .select(`#chart_scaler${String(index)}`)
      .attr('width', width + margin.left + margin.right)
      .attr('height', height + margin.top + margin.bottom)
      .append('g')
      .attr('transform', `translate(${margin.left},${margin.top})`)

    axisTemplate.append('g').append('g').attr('transform', `translate(0,${height})`).call(d3.axisBottom(xScaleLog))
