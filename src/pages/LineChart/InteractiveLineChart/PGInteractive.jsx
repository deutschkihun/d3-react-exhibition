...
export function PGInteractive({ width, height, margin }) {
  useEffect(() => {
    const pgchart = d3
      .select('#pgInteractive')
    ...
    d3.csv('https://raw.githubusercontent.com/deutschkihun/vizDataRepo/main/csv/PG.csv', d => {
      ...
      pgchart.append('g').attr('transform', `translate(0, ${height})`).call(d3.axisBottom(x))

      const y = d3.scaleLinear().domain([60, 200]).range([height, 0])
      pgchart.append('g').call(d3.axisLeft(y))
