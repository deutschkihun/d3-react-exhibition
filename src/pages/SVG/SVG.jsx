  ...
  return (
    <VizWrapper>
      <VizContainer>
        ...
        <Message>
          Scalable Vector Graphics (SVG) are an XML-based markup language for describing two-dimensional based vector
          graphics.SVG is open Web standard for describing images that can be rendered cleanly at any size and are
          designed specifically to work well with other web standards including CSS, DOM, JavaScript, and SMIL. SVG is,
          essentially, to graphics what HTML is to text. SVG images and their related behaviors are defined in XML text
          files, which means they can be searched, indexed, scripted, and compressed. Additionally, this means they can
          be created and edited with any text editor or with drawing software. Compared to classic bitmapped image
          formats such as JPEG or PNG, SVG-format vector images can be rendered at any size without loss of quality and
          can be easily localized by updating the text within them, without the need of a graphical editor to do so.
        </Message>
      </VizContainer>
      ...
      <VizContainer>
        ...
        <OlympicFlag width={400} height={230} centerX={-35} centerY={-35} radius={60} strokeWidth={4} stroke={stroke} />
      </VizContainer>
      ...
      <VizContainer>
        <Title>
          ...
          <RenderButton
            onClick={() =>
              refreshHandler(
                <Face width={width} height={height} centerX={width / 2} centerY={height / 2} type="random" />,
                FaceRef,
              )
            }
          >
      ...
      <VizContainer>
        ...
        <Message>
          In SVG, the position of element is with pixel set. But when we use a input data to set a svg position, we need
          a function that convert numeric variable to a svg position in pixel unit. This is what we called a scale
          function in d3. Scale has 2 important properties. Domain represents the interval of min and max for the data
          percentage. Range represent a interval of pixel.
        </Message>
        <SVGScale width={width} height={height} centerX={width / 2} centerY={height / 2} stroke={stroke} radius={10} />
      </VizContainer>
      <SVGScaleDescription />
